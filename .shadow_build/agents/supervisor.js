    ...
    async execute(goal) {
        ...
        async function runTask(task) {
            ...
            const templateContext = taskGraph.template && taskGraph.template !== "NONE" && TEMPLATES[taskGraph.template] 
                ...
                : `\n[BUILD FROM SCRATCH]: No predefined template used. Follow the blueprint strictly.`;
            ...
            const res = await TargetAgent.think([
                { role: "system", content: `${coreRules}${templateContext}${brandContext}${blueprintContext}` },
        ...
        // 1. Try to find the synthesis result (the master-weave)
        const synthesisTask = taskGraph.tasks.find(t => t.type === 'synthesis');
        let artifact = synthesisTask ? results[synthesisTask.id] : null;

        // 2. Fallback to the final task (Guardian/TechLead) if synthesis is missing or too small
        if (!artifact || artifact.length < 200) {
            const lastTask = taskGraph.tasks[taskGraph.tasks.length - 1];
            artifact = results[lastTask.id];
        }

        // 3. Last resort: Join all outputs
        if (!artifact || artifact.length < 100) {
            console.warn("Supervisor: Target artifact suspiciously small. Falling back to Join strategy.");
            artifact = Object.values(results).join("\n\n");
        }
