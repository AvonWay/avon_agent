...
export async function runGemini({ model, messages, system, signal }) {
    ...
    // Extract system instructions from messages if they exist
    ...
    const contents = conversationMessages.map(m => ({
        role: m.role === 'assistant' ? 'model' : 'user', // Fallback to 'user' for safety
    ...
    if (finalSystem) {
        body.system_instruction = { parts: [{ text: finalSystem }] };
    }
